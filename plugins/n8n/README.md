...
| `webhooks_public` | **boolean** · Required · Default: `false`<br>Allows public access to the n8n webhook paths — the editor, REST API, and credential store stay behind authentication |
...
- With `webhooks_public` disabled (the default) every path is behind the Juno session gate, so calls from third-party services are rejected at the ingress before n8n receives them — enable it for any workflow driven by an inbound webhook
